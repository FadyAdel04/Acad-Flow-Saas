...
export async function updateStudentLevel(studentId: string, newLevel: string): Promise<void> {
  ...
  if (level) {
    ...
    const { assignPreviousLevelMaterials } = await import('./materialAssignmentService');
    await assignPreviousLevelMaterials(studentId, newLevel, assignedBy);
  }
}
...
export async function uploadMaterialMulti(payload: {
  title: string;
  levelIds: string[];
  groupIds: string[];
  description?: string;
  ...
  if (payload.levelIds.length === 0 && payload.groupIds.length === 0) {
    throw new Error('Please select at least one level or group.');
  }
  ...
  // 2. Insert one materials row (use first selected level as primary)
  const primaryLevelId = payload.levelIds[0] ?? null;
  const { data: created, error: dbError } = await supabase
    .from('materials')
    .insert({
      title: payload.title,
  ...
  // 3. Collect all student IDs from selected levels
  ...
  for (const levelId of payload.levelIds) {
    const { data: ls } = await supabase
      ...
      .select('student_id')
      .eq('level_id', levelId);
    (ls ?? []).forEach((r: any) => studentIdSet.add(r.student_id));
  }

  // 4. Collect student IDs from selected groups (via profiles.group_id)
  if (payload.groupIds.length > 0) {
    const { data: gStudents } = await supabase
      .from('profiles')
      .select('id')
      .eq('role', 'student')
      .in('group_id', payload.groupIds);
    (gStudents ?? []).forEach((r: any) => studentIdSet.add(r.id));
  }

  const studentIds = Array.from(studentIdSet);

  // 5. Bulk-assign material to all collected students
  if (studentIds.length > 0) {
    const { data: authData } = await supabase.auth.getUser();
    const { syncMaterialAssignments: sync } = await import('./materialAssignmentService');
    await sync({
      materialId,
      studentIds,
      availableFrom: new Date().toISOString(),
      visible: true,
      assignedBy: authData.user?.id ?? null,
    });
  }
...
export async function fetchMaterialAssignments(materialId: string) {
  const { fetchAssignmentsForMaterial } = await import('./materialAssignmentService');
  return fetchAssignmentsForMaterial(materialId);
}

export async function updateMaterialMulti(payload: {
  id: string;
  title: string;
  levelIds: string[];
  groupIds: string[];
  ...
  const primaryLevelId = payload.levelIds[0] ?? null;
  const updates: { title: string; level_id: string | null; file_url?: string; description?: string; type?: string } = {
    title: payload.title,
    level_id: primaryLevelId,
  ...
  if (studentIds.length > 0) {
    const { data: authData } = await supabase.auth.getUser();
    const { bulkAssignMaterials } = await import('./materialAssignmentService');
    await bulkAssignMaterials({
      materialIds: [payload.id],
      studentIds,
      availableFrom: new Date().toISOString(),
      assignedBy: authData.user?.id ?? null,
    });
...
export async function fetchAllAssignments(): Promise<(Assignment & { levels?: { name: string } })[]> {
  ...
  const { data: assignmentsData, error: assignmentsError } = await supabase
    .from('assignments')
    .select('*, levels(name), instructor:profiles!instructor_id(name)')
    .order('created_at', { ascending: false });
