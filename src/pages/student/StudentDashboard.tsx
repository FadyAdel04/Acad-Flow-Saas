...
function formatDeadline(deadline: string | null) {
  if (!deadline) return 'لا يوجد موعد نهائي';
  return new Date(deadline).toLocaleDateString('ar-EG', { day: 'numeric', month: 'short' });
}
...
export default function StudentDashboard() {
  ...
  const [materialGroups, setMaterialGroups] = useState<GroupedStudentMaterials | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const [grouped, asgns, exs, ress, subs, prog, att] = await Promise.all([
          fetchMyMaterialsGrouped(user!.id, prof.current_level ?? null),
        ...
        if (cancelled) return;
        setMaterialGroups(grouped);
        setMaterials(grouped.all.map((r) => r.material));
        ...
        if (!cancelled) setError(e instanceof Error ? e.message : 'فشل تحميل لوحة التحكم');
      } finally {
