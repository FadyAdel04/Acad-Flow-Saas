...
import { Grid, Users, FileText, Award } from 'lucide-react';
...
export default function InstructorDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F0] lg:flex">
      <InstructorSidebar />
      <motion.div initial="hidden" animate="visible" variants={containerVariants} className="flex-1 lg:mr-80 p-6 md:p-10 lg:p-16 xl:p-20 overflow-hidden relative">
        <div className="absolute top-0 left-0 w-[50vw] h-[50vw] bg-[#D4A373]/[0.02] rounded-full blur-[100px] pointer-events-none -translate-y-1/2 -translate-x-1/2" />
        ...
        <motion.header variants={itemVariants} className="mb-12 relative z-10">
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-black tracking-tighter text-[#1A1A1A] leading-none uppercase mb-3">
            بوابة<br /><span className="text-[#D4A373]">المدرب.</span>
          </h1>
          <p className="text-[#1A1A1A]/40 font-black uppercase text-[10px] tracking-[0.5em] italic flex items-center gap-2">
            ...
            <span className="text-[#D4A373] tracking-widest uppercase">نشط • مدرب معتمد</span>
          </p>
        </motion.header>
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12 relative z-10">
          <div className="bg-white p-8 rounded-[2rem] border border-[#1A1A1A]/5 shadow-sm">
            ...
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-[#1A1A1A]/30 mb-1">المستويات المُسندة</p>
          ...
          <div className="bg-white p-8 rounded-[2rem] border border-[#1A1A1A]/5 shadow-sm">
            ...
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-[#1A1A1A]/30 mb-1">إجمالي الطلاب</p>
          ...
          <div className="bg-white p-8 rounded-[2rem] border border-[#1A1A1A]/5 shadow-sm">
            ...
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-[#1A1A1A]/30 mb-1">الواجبات</p>
          ...
          <div className="bg-white p-8 rounded-[2rem] border border-[#1A1A1A]/5 shadow-sm">
            ...
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-[#1A1A1A]/30 mb-1">متوسط الأداء</p>
        ...
        <motion.div variants={itemVariants} className="space-y-6 relative z-10">
          <div className="flex items-center justify-between px-4">
            <h3 className="text-xl font-black text-[#1A1A1A] uppercase tracking-tighter">مستوياتي</h3>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {loading ? (
              ...
              <div className="col-span-full p-12 bg-white rounded-[2rem] border border-[#1A1A1A]/5 text-center">
                <p className="font-black text-[#1A1A1A]/20 uppercase tracking-widest text-[10px]">لا توجد مستويات مسندة بعد.</p>
              </div>
            ) : (
              levels.map(level => (
                <div key={level.id} className="group bg-white p-8 rounded-[3rem] border border-[#1A1A1A]/5 shadow-sm hover:shadow-2xl transition-all relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-24 h-24 bg-[#D4A373]/5 rounded-full blur-2xl -translate-y-1/2 -translate-x-1/2 group-hover:scale-150 transition-transform" />
                  <div className="flex items-center justify-between mb-8">
                    <div className="px-4 py-1.5 bg-[#F5F5F0] rounded-full text-[9px] font-black uppercase tracking-widest text-[#1A1A1A]/60 group-hover:bg-[#D4A373] group-hover:text-white transition-all">
                      {(level as any).group?.name ? `مجموعة ${(level as any).group.name} — ` : ''}المستوى {level.name}
                    </div>
                    <div className="flex items-center gap-2">
                      <Users className="w-3 h-3 text-[#D4A373]" />
                      <span className="text-[10px] font-black">{level.student_count || 0} طالب</span>
                    </div>
                  </div>
                  ...
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#1A1A1A]/30 italic truncate">
                    {level.description || 'مناهج تعليمية'}
                  </p>
                </div>
              ))
            )}
