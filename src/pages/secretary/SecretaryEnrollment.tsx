...
import { fetchAllStudents, invalidateAdminCache } from '../../services/adminService';
...
export default function SecretaryEnrollment() {
  ...
  const handleEnroll = async (studentId: string) => {
    ...
    try {
      ...
      await supabase
        .from('profiles')
        .update({ current_level: level.name })
        .eq('id', studentId);

      const { data: authData } = await supabase.auth.getUser();
      const { assignPreviousLevelMaterials } = await import('../../services/materialAssignmentService');
      await assignPreviousLevelMaterials(studentId, level.name, authData.user?.id ?? null);
