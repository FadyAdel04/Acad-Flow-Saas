    ...
    <motion.div initial="hidden" animate="visible" variants={cv} className="min-h-screen bg-[#F5F5F0] lg:flex font-body">
      ...
      <main className="pt-14 lg:pt-0 lg:mr-80 flex-1 p-4 sm:p-6 md:p-10 lg:p-16 relative overflow-hidden">
        ...
        <motion.header variants={ci} className="mb-10 lg:mb-14 relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div className="space-y-4">
            <div className="flex items-center gap-3 bg-[#1A1A1A] text-white px-4 py-1.5 rounded-full w-fit">
              ...
              <span className="text-[8px] font-black uppercase tracking-[0.3em]">Secretary · Members</span>
            </div>
            <h1 className="text-4xl sm:text-6xl font-black tracking-tighter text-[#1A1A1A] leading-none uppercase">
              Member<br /><span className="text-[#F97316]">Registry.</span>
            </h1>
          </div>
            ...
            {(['all', 'student', 'instructor'] as const).map(role => (
              <button
                ...
                className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                  roleFilter === role
                    ? role === 'student' ? 'bg-[#F97316] text-white'
                    : 'bg-[#1A1A1A] text-white'
                    : 'hover:bg-[#F5F5F0]'
                }`}
              >
                {role === 'all' ? 'All' : role === 'student' ? 'Students' : 'Instructors'}
              </button>
            ))}
          </div>
        </motion.header>
        ...
        <motion.div variants={ci} className="bg-white rounded-[3rem] border border-[#1A1A1A]/5 shadow-xl overflow-hidden relative z-10">
          {loading
            ...
            : filtered.length === 0
              ? <div className="flex flex-col items-center justify-center py-32 text-center">
                  ...
                  <p className="font-black text-[#1A1A1A] uppercase tracking-tighter text-2xl">No members found</p>
                  <p className="text-[10px] font-black uppercase tracking-widest text-[#1A1A1A]/30 mt-2">Try adjusting your filters</p>
                </div>
              : (
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b border-[#1A1A1A]/5 bg-[#F5F5F0]/30">
                        {['Identity', 'Role', 'Level / Group', 'Joined', 'Actions'].map(h => (
                          <th key={h} className="px-8 py-6 text-[9px] font-black uppercase tracking-[0.3em] text-[#D4A373] whitespace-nowrap">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                          ...
                          <td className="px-8 py-6">
                            <div className="flex items-center gap-4">
                              ...
                              <div className="space-y-1">
                                <p className="font-black text-sm text-[#1A1A1A] uppercase tracking-tight">{u.name || 'Anonymous'}</p>
          ...
          {/* Footer */}
          <div className="px-8 py-6 bg-[#F5F5F0]/30 border-t border-[#1A1A1A]/5 flex justify-between items-center">
            ...
            <div className="flex items-center gap-2 opacity-50">
              ...
              <span className="text-[8px] font-black uppercase tracking-widest text-[#1A1A1A]">Live Sync</span>
            </div>
          </div>
        </motion.div>
      </main>
