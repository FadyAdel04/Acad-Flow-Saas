...
import {
  ...
  FiArrowLeft, FiCheckSquare, FiGrid
} from 'react-icons/fi';
...
export default function SecretaryDashboard() {
    ...
    async function loadStats() {
      try {
        ...
        setStats({
          ...
          levels: lvls.length,
          activeGroups: [...new Set(lvls.map((l: Level) => l.name[0]))].length
        });
      } catch (err) {
        console.error('فشل تحميل الإحصاءات');
      } finally {
        setLoading(false);
      }
    }
  ...
  const cards = [
    { label: 'الطلاب المسجلون', value: stats.students, icon: FiUsers, color: '#F97316', path: '/secretary/enrollment' },
    { label: 'المستويات النشطة', value: stats.levels, icon: FiLayers, color: '#D4A373', path: '/secretary/levels' },
    { label: 'المجموعات', value: stats.activeGroups, icon: FiActivity, color: '#C62828', path: '/secretary/groups' },
  ];
  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={cv} className="min-h-screen bg-[#F5F5F0] lg:flex">
      ...
      <main className="pt-14 lg:pt-0 lg:mr-80 flex-1 p-4 sm:p-6 md:p-10 lg:p-16 xl:p-20 relative overflow-hidden">
        {/* Background Accent */}
        <div className="absolute top-0 left-0 w-[50vw] h-[50vw] bg-[#F97316]/[0.03] rounded-full blur-[100px] pointer-events-none -translate-y-1/2 -translate-x-1/2" />
        ...
        <motion.header variants={ci} className="mb-12 lg:mb-20 space-y-6 relative z-10">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-[#D4A373] italic">طبقة الإدارة الأكاديمية</span>
          </div>
          <h1 className="text-5xl sm:text-7xl font-black tracking-tighter text-[#1A1A1A] leading-[0.9] uppercase">
            أهلاً،<br /><span className="text-[#F97316]">{user?.name?.split(' ')[0] || 'السكرتيرة'}.</span>
          </h1>
          <p className="max-w-xl text-[#1A1A1A]/40 font-black text-[13px] uppercase tracking-widest leading-relaxed italic">
            مرحباً بك في مركز الإدارة. تابع الحضور، وادر توزيع المناهج، وزامن سجلات الطلاب بدقة.
          </p>
        </motion.header>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12 relative z-10">
          {cards.map((card, i) => (
            ...
            >
              <div className="absolute top-0 left-0 w-32 h-32 rounded-full -translate-y-1/2 -translate-x-1/2 blur-2xl opacity-5 group-hover:opacity-10 transition-all" style={{ background: card.color }} />
              ...
              <div className="mt-8 flex items-center gap-2 text-[#F97316] font-black text-[9px] uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0 relative z-10">
                الدخول للإدارة <FiArrowLeft className="w-3 h-3" />
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <motion.div variants={ci} className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
          ...
          >
            <div className="absolute inset-0 bg-gradient-to-bl from-[#F97316]/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
            <div className="relative z-10">
              ...
              <h3 className="text-4xl font-black tracking-tighter uppercase leading-none mb-4">
                تسجيل<br /><span className="text-[#F97316]">الحضور</span> اليومي
              </h3>
              <p className="text-xs font-black text-white/30 uppercase tracking-[0.2em] max-w-xs leading-relaxed">
                زامن سجلات الحضور لجلسات المستويات النشطة فوراً.
              </p>
            </div>
            <div className="flex items-center gap-4 relative z-10">
              <div className="flex-1 h-[1px] bg-white/10" />
              <span className="text-[10px] font-black uppercase tracking-[0.4em] text-[#F97316]">فتح الشبكة</span>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {[
              { label: 'إدارة المجموعات', icon: FiGrid, path: '/secretary/groups', desc: 'توزيع الطلاب على المجموعات الدراسية' },
              { label: 'هيكل المستويات', icon: FiLayers, path: '/secretary/levels', desc: 'مراقبة توزيع المستويات والمدربين' },
              { label: 'تسجيل الطلاب', icon: FiUsers, path: '/secretary/enrollment', desc: 'تسجيل طلاب جدد في المستويات' }
            ].map((act, i) => (
              <div
                key={i}
              ...
              >
                ...
                <FiArrowLeft className="w-4 h-4 text-[#1A1A1A]/10 group-hover:text-[#F97316] group-hover:-translate-x-2 transition-all" />
              </div>
            ))}
          </div>
        </motion.div>
