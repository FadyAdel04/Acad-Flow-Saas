...
export default function TopNavBar() {
  ...
  const navItems = [
    ...
    { name: 'لوحات التحكم', id: 'roles' },
    { name: 'جرب النظام', id: 'demo' },
  ...
  if (resolvedRole === 'admin') { portalLabel = 'لوحة الإدارة'; portalPath = '/admin'; }
  else if (resolvedRole === 'instructor') { portalLabel = 'لوحة المدرس'; portalPath = '/instructor'; }
  else if (resolvedRole === 'secretary') { portalLabel = 'لوحة السكرتارية'; portalPath = '/secretary/attendance'; }
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 w-full z-100 bg-[#F5F5F0]/80 backdrop-blur-xl border-b border-[#1A1A1A]/5" dir="rtl">
      <div className="max-w-[1440px] mx-auto px-4 md:px-12 h-16 md:h-20 lg:h-24 flex justify-between items-center relative">
        ...
        >
          ...
          />
          <div className="hidden md:flex items-center mr-3 lg:mr-4 relative">
            <div className="relative px-4 py-2 rounded-2xl overflow-hidden">
              ...
              />
              <span className="relative z-10 text-[#C62828] text-xl lg:text-2xl font-black tracking-tighter drop-shadow-sm">Acad</span>
              <span className="relative z-10 text-[#1A1A1A] ml-0.5 lg:ml-1 text-xl lg:text-2xl font-black tracking-tighter drop-shadow-sm">Flow</span>
            </div>
          </div>
          <div className="md:hidden relative">
            <div className="w-10 h-10 rounded-full overflow-hidden shadow-md group-hover:shadow-lg transition-all">
              <img src={logo} alt="AcadFlow LMS" width={40} height={40} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
            </div>
          </div>
        </motion.div>

        {/* Desktop Nav */}
        <div className="hidden lg:flex gap-6 xl:gap-8 items-center bg-white/40 backdrop-blur-md px-6 xl:px-8 py-3 rounded-full border border-[#1A1A1A]/5 shadow-sm">
          {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => handleScroll(item.id)}
              className={`font-black text-[11px] uppercase tracking-[0.15em] transition-all relative py-1 px-1 group ${
                location.pathname === '/' && activeSection === item.id ? 'text-[#C62828]' : 'text-[#1A1A1A]/40'
              } hover:text-[#C62828]`}
            >
              {item.name}
              {location.pathname === '/' && activeSection === item.id && (
                <motion.div layoutId="activeTab" className="absolute -bottom-1 left-0 right-0 h-0.5 bg-[#C62828] rounded-full" />
              )}
            </button>
          ))}
        </div>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="w-full flex justify-between items-center pb-4 border-b border-[#1A1A1A]/10 mb-4">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-full overflow-hidden bg-[#F5F5F0] shadow-md">
                  <img src={logo} alt="" width={48} height={48} className="w-full h-full object-cover" />
                </div>
                <div>
                  <span className="text-base font-black text-[#C62828] block">AcadFlow</span>
                  <span className="text-xs font-black text-[#1A1A1A]/40">نظام إدارة تعليمي</span>
                </div>
              </div>
              <button
            ...
            <div className="w-full space-y-1">
              {navItems.map((item) => (
                <motion.button
                  ...
                  onClick={() => handleScroll(item.id)}
                  className={`w-full text-right px-6 py-4 rounded-xl font-black text-base tracking-tight transition-all relative ${
                ...
                >
                  {item.name}
                  {activeSection === item.id && (
                    <motion.div layoutId="mobileActiveTab" className="absolute right-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-[#C62828] rounded-l-full" />
                  )}
                </motion.button>
              ))}
            </div>
            ...
            <div className="w-full pt-6 mt-4 border-t border-[#1A1A1A]/10 space-y-3">
              {deferredPrompt && (
                <motion.button
                  ...
                  onClick={handleInstallApp}
                  className="w-full py-4 text-[#C62828] font-black tracking-wider text-sm border-2 border-[#C62828] rounded-2xl hover:bg-[#C62828] hover:text-white transition-all"
                >
                  تثبيت التطبيق
                </motion.button>
              )}
              {isSignedIn ? (
                <motion.button
                  ...
                  className="w-full py-4 bg-[#C62828] text-white font-black tracking-wider text-sm rounded-2xl shadow-xl shadow-[#C62828]/20 hover:shadow-2xl hover:-translate-y-0.5 transition-all"
                >
                  {portalLabel}
                </motion.button>
              ) : (
                <>
                  <motion.button
                    ...
                    className="w-full py-4 text-[#1A1A1A] font-black tracking-wider text-sm border-2 border-[#1A1A1A] rounded-2xl hover:bg-[#1A1A1A] hover:text-white transition-all"
                  >
                    تسجيل الدخول
                  </motion.button>
                  <motion.button
                    initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.35 }}
                    onClick={() => { handleScroll('demo'); setIsOpen(false); }}
                    className="w-full py-4 bg-[#C62828] text-white font-black tracking-wider text-sm rounded-2xl shadow-xl shadow-[#C62828]/20 hover:shadow-2xl hover:-translate-y-0.5 transition-all"
                  >
                    جرب النظام
                  </motion.button>
