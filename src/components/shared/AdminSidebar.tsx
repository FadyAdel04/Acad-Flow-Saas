...
const navLinks = [
  { to: '/admin', label: 'لوحة التحكم', icon: RiDashboardLine, exact: true },
  { to: '/admin/students', label: 'الطلاب', icon: FiUsers },
  { to: '/admin/groups', label: 'المجموعات', icon: FiGrid },
  { to: '/admin/levels', label: 'المستويات', icon: FiMenu },
  { to: '/admin/materials', label: 'المواد التعليمية', icon: FiBookOpen },
  { to: '/admin/material-assignments', label: 'تعيين المواد', icon: FiCheckSquare },
  { to: '/admin/assignments', label: 'الواجبات', icon: FiFileText },
  { to: '/admin/exams', label: 'الاختبارات', icon: FiAward },
  { to: '/admin/attendance', label: 'الحضور والغياب', icon: FiCheckSquare },
  { to: '/admin/website', label: 'إعدادات الموقع', icon: FiImage },
  { to: '/admin/notifications', label: 'الإشعارات', icon: FiBell },
];
...
export default function AdminSidebar() {
  ...
  const adminName = adminInfo?.userMetadata?.name as string || adminInfo?.email?.split('@')[0] || 'المدير';
    ...
    <>
      ...
      <div className="lg:hidden fixed top-0 right-0 left-0 z-60 bg-[#1A1A1A] border-b border-white/5 h-14 flex items-center justify-between px-4">
        <span onClick={() => navigate('/')} className="text-lg font-black text-white tracking-tighter cursor-pointer">
          Acad Flow <span className="text-[#D4A373]">مدير</span>
        </span>
      ...
      <aside className={`h-screen w-72 lg:w-80 fixed right-0 top-0 bg-[#1A1A1A] flex flex-col py-8 lg:py-12 border-l border-white/5 shadow-2xl z-80 overflow-hidden transition-transform duration-300 ${open ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'}`}>
        ...
        <div className="px-8 lg:px-12 mb-8 lg:mb-12 relative z-10">
          <span onClick={() => navigate('/')} className="text-2xl lg:text-3xl font-black text-white tracking-tighter block leading-none cursor-pointer hover:opacity-80 transition-opacity">
            Acad Flow<br /><span className="text-[#D4A373]">المدير.</span>
          </span>
          <p className="text-[10px] font-black uppercase tracking-[0.5em] text-[#F97316] mt-3">لوحة الإدارة</p>
        </div>
        ...
        <div className="px-6 lg:px-8 mb-8 relative z-10">
          <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/10">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] font-black uppercase tracking-widest text-[#F97316] mt-0.5 italic">مدير النظام</p>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 flex flex-col gap-1.5 px-4 lg:px-6 relative z-10 overflow-y-auto">
          {navLinks.map(({ to, label, icon: Icon, exact }) => (
            <Link
              ...
              to={to}
              onClick={() => setOpen(false)}
              className={`flex items-center gap-4 px-6 py-4 rounded-2xl font-black text-sm transition-all ${isActive(to, exact)
